...
design_matrix <- model.matrix(Outcome ~ ., data = train)
response_variable <- train$Outcome

set.seed(123)
rf_model <- randomForest(design_matrix, response_variable, ntree = 500, mtry = 2)
```
...
roc_rf <- roc(response = test$Outcome, predictor = pred_rf)

# Imprimir el valor del área bajo la curva ROC
cat("Área bajo la curva ROC: ", roc_rf$auc)
...
cat("Este es el resultado de evaluar el modelo en el conjunto de datos de entrenamiento:\n\n")
matriz_confusion
```
```{r}
...
```{r}
glm.probs <- predict(log_reg_model, newdata = test, type = "response")
